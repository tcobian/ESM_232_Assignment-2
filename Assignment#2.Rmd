...
almond_yield <- function( clim_data = clim,
  ...
  clim_data = ifelse(class(clim_data) != "data.frame", return("Climate data input must be a data frame"), clim_data)
  
   # Make sure the climate data input contains the columns year, month, precip, tmin_c and tmax_c
 clim_data = ifelse(all(has_name(clim_data, c("year", "month", "precip", "tmax_c", "tmin_c"))) != TRUE, return ("Climate data input must contain the following columns: year, month, precip, tmin_c and tmax_c"), clim_data)
...
```{r}
# Create sample
sample <- rnorm( n = 5, mean = 0.0043, sd = 0.00043)
